...
if($_POST){
  ...
  if(empty($_GET['fun_id'])){
    ...
    $Tb_index='ot'.date('YmdHis').rand(0,99);
    ...
    location_up('iframe_other.php?Tb_index='.$_GET['Tb_index'].'&fun_id='.$Tb_index, '功能已成功新增');
  }
  ...
  //----------------- 修改 ----------------------------------------------
    ...
    location_up('iframe_other.php?Tb_index='.$_GET['Tb_index'].'&fun_id='.$Tb_index, '功能已更新');
  }
  ...
  $row=pdo_select("SELECT * FROM other_tb WHERE Tb_index=:Tb_index", ['Tb_index'=>$_GET['fun_id']]);
?>

<div class="wrapper wrapper-content animated fadeInRight">
	<div class="col-lg-12">
		<h2 class="text-primary"><?php echo $row_case['aTitle'];?>-自定義</h2>
