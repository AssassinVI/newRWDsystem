<?php 
  ...
  $case=pdo_select('SELECT * FROM build_case WHERE Tb_index=:Tb_index', ['Tb_index'=>'case'.$Tb_index[2]] );
  //廣告製作
  ...
  //導航連結
  $gps_row=pdo_select('SELECT location FROM googlemap_tb WHERE case_id=:case_id', ['case_id'=>'case'.$Tb_index[2]] );
  $map_txt= empty($gps_row['location']) ? $case['build_adds'] : $gps_row['location'];

  // 功能區塊
  $fun_block_id=pdo_select('SELECT fun_id FROM Related_tb WHERE case_id=:case_id ORDER BY OrderBy ASC' , ['case_id'=>'case'.$Tb_index[2]] );
